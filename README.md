...
In this final milestone, the architecture of the application is now designed to handle concurrent HTTP requests by implementing a robust ThreadPool. Instead of spawning a new thread for every single incoming connection, the pool pre-allocates a fixed size of four worker threads that are ready to process tasks.
To effectively distribute the incoming TCP streams from the main thread to these workers, a multi-producer, single-consumer (mpsc) channel is established. Since all four worker threads need to safely read from the exact same receiving end of this channel, the receiver is wrapped in an Arc (Atomic Reference Counted) smart pointer combined with a Mutex.
The Arc allows multiple threads to hold shared ownership of the receiver, while the Mutex guarantees thread safety by ensuring only one worker can lock the queue and extract a job at any given moment. Inside main.rs, the pool.execute method successfully passes the connection-handling closure down this channel to be picked up by the next available worker.
As a result, this multithreaded architecture resolves the bottleneck present in a single thread architecture. Even if one worker needs to process the 10 second delay caused by the /sleep endpoint, the remaining workers are still free to serve the hello.html page to other concurrent users quickly.

# Commit Bonus Reflection Notes
To fulfill the bonus requirement, the initialization logic of the ThreadPool was significantly improved to follow to Rust's paradigm of safe error handling.
In the previous version, the implementation of ThreadPool::new used an assert! macro that would cause an unrecoverable panic if the user attempted to instantiate a pool with zero threads, abruptly crashing the entire application.
The solution to this is to write a custom error type named PoolCreationError and to replace the new function with a build method. This build method now safely returns a Result<ThreadPool, PoolCreationError>, shifting the responsibility of handling the zero-size edge case back to the caller.
Inside main.rs, the unwrap_or_else method is used to intercept this Result. If an error is detected, the application now prints a descriptive error message to the standard error stream and executes a graceful process::exit(1) which eliminates the uncontrolled panic behaviour.
This refactoring demonstrates a much more resilient approach to software design by anticipating and safely managing invalid instantiation states.